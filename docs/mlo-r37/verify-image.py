#!/usr/bin/env python3
"""Verify the r37 pending-mailbox preservation repair; writes only to a new extraction directory.

Usage: verify-image.py BUILD IMAGE NEW_OUTPUT MANIFEST --baseline R36_ARTIFACT_DIR
Uses the existing fwtool/dumpimage/unsquashfs tools and verified r36 baseline.
...
def verify(build, image, out, manifest_path, previous):
    ...
    require(sha256(previous / 'build-manifest.json') == manifest['base_manifest_sha256'],
            'r36 baseline manifest hash mismatch')
    require(old_manifest['build_label'] == manifest['base_build'] ==
            'ubi2-mlo-tx-publish-r36-20260926-tailscale', 'Wrong r36 baseline build')
    ...
    require(manifest['build_label'] == 'ubi2-mlo-mailbox-r37-20260926-tailscale', 'Wrong r37 build label')
    ...
    require(required_sources <= set(manifest['changed_source_sha256']),
            'Mailbox or inherited TX publication patch/regression source missing from manifest')
    ...
    require(baseline['kmod-airoha-npu']['version'] == '6.18.44-r2',
            'Wrong baseline NPU package version')
    require(packages['kmod-airoha-npu']['version'] == '6.18.44-r3', 'NPU package not upgraded')
    ...
    require(kernel_abi == baseline['kernel']['version'], 'Kernel ABI differs from r36')
    ...
    require({path.name for path in (old_root / 'lib/modules' / old_manifest['kernel_release']).glob('*.ko')} == set(module_hashes),
            'Module inventory differs from r36')
    ...
    require(changed_modules == {'airoha_npu.ko'},
    ...
    for package, module in [('kmod-airoha-npu', 'airoha_npu.ko'),
    ...
    result = {'result': 'PASS', 'image': image.name, 'sha256': entry['sha256'],
              ...
              'changed_modules': sorted(changed_modules), 'kernel_payload_identical_to_r36': True,
...
def main():
    ...
    parser.add_argument('--baseline', type=Path, required=True, help='Verified r36 artifact directory, including manifest and extracted image')
