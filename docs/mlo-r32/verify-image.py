#!/usr/bin/env python3
"""Verify a completed r32 ubi2 image; writes only to a new extraction directory.

Usage: verify-image.py BUILD IMAGE NEW_OUTPUT MANIFEST --baseline R31_ARTIFACT_DIR
Uses the existing fwtool/dumpimage/unsquashfs tools and verified r31 baseline.
...
def verify(build, image, out, manifest_path, previous):
    ...
    require(sha256(previous / 'build-manifest.json') == manifest['base_manifest_sha256'],
            'r31 baseline manifest hash mismatch')
    ...
    require(manifest['build_label'] == 'ubi2-mlo-reconnect-r32-20260924-tailscale', 'Wrong r32 build label')
    ...
    for field in ('feed_patch_sha256', 'feed_recipe_overrides_sha256'):
        for name, digest in manifest[field].items():
    ...
    require(set(packages) == set(baseline) | ({'ip-bridge'} if packaging_only else set()), 'Unexpected installed package names')
    ...
    allowed = {'base-files','bridge-flow-offload'} if packaging_only else {'base-files','netifd','kernel'}
    require(all(name in allowed or (not packaging_only and name.startswith('kmod-')) for name in changes),
    ...
    require((kernel_abi == baseline['kernel']['version']) if packaging_only else (kernel_abi != baseline['kernel']['version']), 'Unexpected kernel ABI relationship to baseline')
    ...
    require({path.name for path in (old_root / 'lib/modules' / old_manifest['kernel_release']).glob('*.ko')} == set(module_hashes),
            'Module inventory differs from r31')

    if packaging_only:
        require(same_file(previous/'verified-image/kernel.gz', out/'kernel.gz'), 'Packaging-only kernel payload changed')
        for name in module_hashes:
            require(same_file(old_root/'lib/modules'/release/name, module_root/name), f'Packaging-only module changed: {name}')
    # r32 retains the reviewed r31 bridge package unchanged.
    require(packages['bridge-flow-offload']['version']=='1.0-r2' and packages['ip-bridge']['version']=='6.18.0-r2', 'Wrong final package versions')
...
def main():
    ...
    parser.add_argument('--baseline', type=Path, required=True, help='Verified r31 artifact directory, including manifest and extracted image')
