#!/usr/bin/env python3
"""Verify the r38 PPE SRAM initialization error propagation repair; writes only to a new extraction directory.

Usage: verify-image.py BUILD IMAGE NEW_OUTPUT MANIFEST --baseline R37_ARTIFACT_DIR
Uses the existing fwtool/dumpimage/unsquashfs tools and verified r37 baseline.
...
def verify(build, image, out, manifest_path, previous):
    ...
    require(sha256(previous / 'build-manifest.json') == manifest['base_manifest_sha256'],
            'r37 baseline manifest hash mismatch')
    require(old_manifest['build_label'] == manifest['base_build'] ==
            'ubi2-mlo-mailbox-r37-20260926-tailscale', 'Wrong r37 baseline build')
    ...
    require(manifest['build_label'] == 'ubi2-mlo-ppe-init-r38-20260926-tailscale', 'Wrong r38 build label')
    ...
    allowed = {'base-files', 'kmod-airoha-eth'}
    ...
    require(baseline['kmod-airoha-eth']['version'] == '6.18.44-r1',
            'Wrong baseline Ethernet package version')
    require(packages['kmod-airoha-eth']['version'] == '6.18.44-r2', 'Ethernet package not upgraded')
    require(packages['kmod-airoha-npu']['version'] == baseline['kmod-airoha-npu']['version'] == '6.18.44-r3', 'NPU package changed')
    require(packages['kmod-mt7996e']['version'] == '6.18.44.2026.09.01~01367e60-r29', 'Wrong mt76 package version')
    for name in mt76_packages:
        require(baseline[name]['version'].endswith('-r29') and
                packages[name]['version'] == baseline[name]['version'],
                f'{name}: mt76 package revision changed from r29')
    ...
    require(kernel_abi == baseline['kernel']['version'], 'Kernel ABI differs from r37')
    ...
    require({path.name for path in (old_root / 'lib/modules' / old_manifest['kernel_release']).glob('*.ko')} == set(module_hashes),
            'Module inventory differs from r37')
    ...
    require(changed_modules == {'airoha-eth.ko'},
    ...
    result = {'result': 'PASS', 'image': image.name, 'sha256': entry['sha256'],
              ...
              'changed_modules': sorted(changed_modules), 'kernel_payload_identical_to_r37': True,
...
def main():
    ...
    parser.add_argument('--baseline', type=Path, required=True, help='Verified r37 artifact directory, including manifest and extracted image')
