#!/usr/bin/env python3
"""Verify a completed r33 ubi2 image; writes only to a new extraction directory.

Usage: verify-image.py BUILD IMAGE NEW_OUTPUT MANIFEST --baseline R32_ARTIFACT_DIR
Uses the existing fwtool/dumpimage/unsquashfs tools and verified r32 baseline.
...
def verify(build, image, out, manifest_path, previous):
    ...
    require(sha256(previous / 'build-manifest.json') == manifest['base_manifest_sha256'],
            'r32 baseline manifest hash mismatch')
    ...
    require(manifest['build_label'] == 'ubi2-mlo-bridge-compat-r33-20260924-tailscale', 'Wrong r33 build label')
    ...
    require(changes.get('bridge-flow-offload') == {'baseline': '1.0-r2', 'candidate': '1.0-r3'}, 'Compatibility package not upgraded')
    ...
    require({path.name for path in (old_root / 'lib/modules' / old_manifest['kernel_release']).glob('*.ko')} == set(module_hashes),
            'Module inventory differs from r32')
    ...
    # r33 corrects only the bridge package compatibility guard.
...
def main():
    ...
    parser.add_argument('--baseline', type=Path, required=True, help='Verified r32 artifact directory, including manifest and extracted image')
