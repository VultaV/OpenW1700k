#!/usr/bin/env python3
"""Verify the r39 NPU TX aggregation inactivity refresh repair; writes only to a new extraction directory.

Usage: verify-image.py BUILD IMAGE NEW_OUTPUT MANIFEST --baseline R38_ARTIFACT_DIR
Uses the existing fwtool/dumpimage/unsquashfs tools and verified r38 baseline.
...
def verify(build, image, out, manifest_path, previous):
    ...
    require(sha256(previous / 'build-manifest.json') == manifest['base_manifest_sha256'],
            'r38 baseline manifest hash mismatch')
    require(old_manifest['build_label'] == manifest['base_build'] ==
            'ubi2-mlo-ppe-init-r38-20260926-tailscale', 'Wrong r38 baseline build')
    ...
    require(manifest['build_label'] == 'ubi2-mlo-npu-ba-r39-20260926-tailscale', 'Wrong r39 build label')
    ...
    required_sources = {
        'package/kernel/mt76/patches/0035-mt7996-refresh-npu-tx-aggregation-timer.patch',
        'tests/test_mt7996_npu_aggregation.py',
        'package/kernel/mt76/patches/0034-wifi-mt76-npu-order-tx-descriptor-publication.patch',
        'tests/test_mt76_npu_rx.py',
        'target/linux/airoha/patches-6.18/9999-z4-net-airoha-npu-preserve-pending-mailbox.patch',
        'tests/test_airoha_npu_mailbox.py',
        'target/linux/airoha/patches-6.18/9999-z5-net-airoha-ppe-propagate-sram-flush-error.patch',
        'tests/test_airoha_ppe_ownership.py',
    }
    require(required_sources <= set(manifest['changed_source_sha256']),
            'PPE SRAM, mailbox or inherited TX publication patch/regression source missing from manifest')
    ...
    mt76_packages = {name for name, item in baseline.items()
                     if '2026.09.01~01367e60-' in item['version']}
    allowed = {'base-files'} | mt76_packages
    require({'kmod-mt76-core', 'kmod-mt7996e'} <= mt76_packages,
            'Required mt76 packages missing from baseline')
    require(all(name in allowed for name in changes),
            f'Unexpected package version changes: {sorted(changes)}')
    require(baseline['kmod-airoha-eth']['version'] == '6.18.44-r2',
            'Wrong baseline Ethernet package version')
    require(packages['kmod-airoha-eth']['version'] == '6.18.44-r2', 'Ethernet package changed')
    require(packages['kmod-airoha-npu']['version'] == baseline['kmod-airoha-npu']['version'] == '6.18.44-r3', 'NPU package changed')
    require(packages['kmod-mt7996e']['version'] == '6.18.44.2026.09.01~01367e60-r30', 'Wrong mt76 package version')
    for name in mt76_packages:
        require(baseline[name]['version'].endswith('-r29') and
                packages[name]['version'] == baseline[name]['version'][:-3] + 'r30',
                f'{name}: mt76 package revision did not advance from r29 to r30')
    ...
    require(kernel_abi == baseline['kernel']['version'], 'Kernel ABI differs from r38')
    ...
    require(len(modules) == 74, 'Expected 74 kernel modules in image')
    ...
    require({path.name for path in (old_root / 'lib/modules' / old_manifest['kernel_release']).glob('*.ko')} == set(module_hashes),
            'Module inventory differs from r38')
    ...
    for package, module in [('kmod-airoha-eth', 'airoha-eth.ko'),
                            ('kmod-airoha-npu', 'airoha_npu.ko'),
                            ('kmod-mt7996e', 'mt7996e.ko'), ('kmod-mt76-core', 'mt76.ko')]:
    ...
    result = {'result': 'PASS', 'image': image.name, 'sha256': entry['sha256'],
              ...
              'changed_modules': sorted(changed_modules), 'kernel_payload_identical_to_r38': True,
...
def main():
    ...
    parser.add_argument('--baseline', type=Path, required=True, help='Verified r38 artifact directory, including manifest and extracted image')
