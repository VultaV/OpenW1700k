...
2026-09-26. r38 이후 정상 데이터 경로를 조사해 **NPU 가속 전송의 TX aggregation inactivity 갱신 누락**을 확인했다. 상대가 0이 아닌 ADDBA timeout을 협상하고, 드라이버가 받는 TXS 보고가 그 timeout보다 자주 도착하는 경우에 해당한다. 이 조건에서 원본은 계속된 전송 활동을 세션 유휴로 판단할 수 있다. r32 실패 회차의 실제 ADDBA timeout은 기록하지 않았으므로 이 수정이 관측된 순간 멈춤의 원인이나 해결이라고 확정하지 않는다.
...
[9월 27일 후속 확인](OFFLINE_EVIDENCE_20260927.md): r29 Air 정상 회차의 수신 ADDBA 13건은 timeout0이며, DELBA 방향까지 대조했다. r32 실패 회차의 협상값은 여전히 미계측이다.
