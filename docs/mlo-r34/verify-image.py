#!/usr/bin/env python3
"""Verify the r34 module/UI repair; writes only to a new extraction directory.

Usage: verify-image.py BUILD IMAGE NEW_OUTPUT MANIFEST --baseline R33_ARTIFACT_DIR
Uses the existing fwtool/dumpimage/unsquashfs tools and verified r33 baseline.
...
def verify(build, image, out, manifest_path, previous):
    ...
    require(sha256(previous / 'build-manifest.json') == manifest['base_manifest_sha256'],
            'r33 baseline manifest hash mismatch')
    ...
    require(manifest['build_label'] == 'ubi2-mlo-link-lifecycle-r34-20260926-tailscale', 'Wrong r34 build label')
    ...
    allowed = {'base-files', 'kmod-airoha-npu', 'luci-app-attendedsysupgrade'}
    mt76_packages = {name for name, item in baseline.items()
                     if '2026.09.01~01367e60-r26' in item['version']}
    ...
    require(packages['kmod-mt7996e']['version'] == '6.18.44.2026.09.01~01367e60-r27', 'mt76 package not upgraded')
    for name in mt76_packages:
        require(packages[name]['version'] == baseline[name]['version'].replace('-r26', '-r27'),
    ...
    require(kernel_abi == baseline['kernel']['version'], 'Kernel ABI differs from r33')
    ...
    require({path.name for path in (old_root / 'lib/modules' / old_manifest['kernel_release']).glob('*.ko')} == set(module_hashes),
            'Module inventory differs from r33')
    ...
    require(changed_modules == {'airoha_npu.ko', 'mt7996e.ko'},
    ...
    for name in preserved_packages:
        ...
        for path in baseline[name]['files']:
            if path == changed_ui:
                require(not same_file(old_root / path, root / path), 'Updater UI did not change')
                continue
    ...
    result = {'result': 'PASS', 'image': image.name, 'sha256': entry['sha256'],
              ...
              'changed_modules': sorted(changed_modules), 'kernel_payload_identical_to_r33': True,
...
def main():
    ...
    parser.add_argument('--baseline', type=Path, required=True, help='Verified r33 artifact directory, including manifest and extracted image')
