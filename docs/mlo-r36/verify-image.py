#!/usr/bin/env python3
"""Verify the r36 TX descriptor publication repair; writes only to a new extraction directory.

Usage: verify-image.py BUILD IMAGE NEW_OUTPUT MANIFEST --baseline R35_ARTIFACT_DIR
Uses the existing fwtool/dumpimage/unsquashfs tools and verified r35 baseline.
...
def verify(build, image, out, manifest_path, previous):
    ...
    require(sha256(previous / 'build-manifest.json') == manifest['base_manifest_sha256'],
            'r35 baseline manifest hash mismatch')
    ...
    require(manifest['build_label'] == 'ubi2-mlo-tx-publish-r36-20260926-tailscale', 'Wrong r36 build label')
    ...
    require(required_sources <= set(manifest['changed_source_sha256']),
            'TX publication patch or regression source missing from manifest')
    ...
    mt76_packages = {name for name, item in baseline.items()
                     if '2026.09.01~01367e60-r28' in item['version']}
    ...
    require(packages['kmod-mt7996e']['version'] == '6.18.44.2026.09.01~01367e60-r29', 'mt76 package not upgraded')
    for name in mt76_packages:
        require(packages[name]['version'] == baseline[name]['version'].replace('-r28', '-r29'),
    ...
    require(kernel_abi == baseline['kernel']['version'], 'Kernel ABI differs from r35')
    ...
    require({path.name for path in (old_root / 'lib/modules' / old_manifest['kernel_release']).glob('*.ko')} == set(module_hashes),
            'Module inventory differs from r35')
    ...
    require(changed_modules == {'mt76.ko'},
    ...
    result = {'result': 'PASS', 'image': image.name, 'sha256': entry['sha256'],
              ...
              'changed_modules': sorted(changed_modules), 'kernel_payload_identical_to_r35': True,
...
def main():
    ...
    parser.add_argument('--baseline', type=Path, required=True, help='Verified r35 artifact directory, including manifest and extracted image')
